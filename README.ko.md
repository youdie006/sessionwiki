...
| `stats` | 전체 합계 + 툴별·월별 분포 + 세션에 연결된 파일 수. |
...
- 아카이브 모드 &mdash; 툴이 원본을 지워도 sessionwiki에 남겨, 그 세션에 대해서도 검색·`brief`·`trace`가 계속 작동하게. 일찍 설치할수록 잃는 게 없고, 툴이 오래전에 지운 대화까지 코드에서 거슬러 갈 수 있습니다.
